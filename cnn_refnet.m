function cnn_refnet(varargin)
% CNN_REFNET   Runs CNN on the reference network.

% There should be two links in the current directory:
%
% matconvnet should link to the matconvnet-1.0-beta16 directory.
% Be sure to compile matconvnet with GPU support using
% vl_compilenn('enableGpu', true)
%
% data should link to the miniplaces root directory, for example
% /mnt/bulk900/share/miniplaces
...
opts.expDir = fullfile('exp', sprintf('miniplaces-%s-%s', ...
...
if exist(opts.imdbPath)
  ...
  imdb = cnn_miniplaces_setup_data('dataDir', opts.dataDir, 'lite', opts.lite) ;
  mkdir(opts.expDir) ;
...
im = cnn_miniplaces_get_batch(images, opts, ...
                            'prefetch', nargout == 0) ;
